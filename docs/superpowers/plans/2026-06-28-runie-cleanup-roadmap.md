...
**Architecture:** The second-pass review showed that several "todo" tasks were already completed on disk (dialog module repaired, empty facade crates deleted, dead provider code removed, IPC layers gone). Those tasks have been archived. A third review found that many small `runie-core` helpers (`glob`, `fuzzy`, `path`, keybinding parsing, frontmatter scanning, tool-marker stripping) can be replaced by `pulldown-cmark`, `strum`, `shellexpand`, `ignore`, and other standard crates. A fourth review found additional crate replacements in provider/config/auth (`backon`, `keyring`, `jsonschema`, `dotenvy`), CLI (`clap`), TUI widgets (`tui-textarea`, `tui-input`, `ansi_colours`), and tooling (`shell-words`, Clippy/CI). A fifth review found broken CI feature flags, dead MCP feature flag, dependency hygiene issues, library-wide `anyhow` usage without typed errors, missing `tracing` subscriber, custom telemetry with no backend, broken ACP plumbing, a still-existing `runie-protocol` crate, and many macros/small parsers that should use `strum`. **As of 2026-06-29, all 108 cleanup tasks from the original architecture review are `done` and the workspace passes `cargo check --workspace` and `cargo test --workspace` cleanly.** The remaining 3 tasks are marked `wontfix` (tool-marker stripping intentionally string-based, pre-release `notify` dependency issue, `palette` adoption deferred). Live mock/MiniMax TUI testing on 2026-06-30, three five-round architecture/code reviews, a deep crate-replacement analysis, and a planned Grok Build comparison uncovered additional bugs and parity gaps; those are tracked as active `todo` tasks plus `partial` tasks in `tasks/index.json`. As of 2026-07-03, 449 tasks are `done`, 27 are `wontfix`, 0 are `partial`, 0 are `blocked`, and 0 are `todo` (total 476 tasks). Workspace passes `cargo check --workspace` and `cargo test --workspace`. Workspace passes `cargo check --workspace` and `cargo test --workspace` (738 tests). Recent closure: feature-gate-heavy-runie-core-subsystems (added 6 optional feature flags: `git`, `watch`, `keyring`, `clipboard`, `mcp`, `tiktoken`; all dependencies now optional and conditionally compiled; each feature compiles independently; workspace passes with all/default/minimal/none feature combinations). Recent closure: adopt-camino-utf8pathbuf-for-internal-paths (migrated 6 internal path fields from `PathBuf` to `Utf8PathBuf`, eliminated `to_string_lossy()` calls on internal paths, added 5 new unit tests for path round-trip). Recent closure: wire-modelclient-turnsession-into-provider-factory-or-delete (deleted dead `ModelClient`/`TurnSession` module from `runie-provider`; `BuiltProvider::cached_http_client` already handles connection reuse). Recent closure: sweep-duplicate-tasks (consolidated 5 execute-*pass* roadmap tasks into create-unified-architecture-backlog-execution-task.md, marked both done). Recent closure: enforce-observed-async-work-in-all-actors (added orphan spawn lint to build.rs). Recent closure: split-ractor-turn-actor-into-focused-modules (split 561-line ractor_turn.rs into handlers.rs, actor.rs, and ractor_turn.rs). Recent closure: replay-sessions-via-events-through-appstate (session replay routes through AppState helper methods instead of direct mutations; removed exemption from build.rs).

Recent closure: restructure-runieerror-with-typed-variants (deleted unused RunieError/RunieErrorKind; typed errors ModelError/ProviderError already in use).

Recent closures: use-tui-popup-for-popup-shell (added `tui-popup` dependency; replaced manual popup shell in path_suggestions, render_welcome, and permission_dialog; kept panel shell for scroll complexity; 733 tests pass). use-tui-scrollview-for-message-feed (marked wontfix; current Paragraph::new().scroll() approach is simpler and more appropriate for the message feed use case). Plan mode TUI integration: added PlanModeEnabled/PlanModeDisabled events, ViewState plan mode fields, Snapshot plan fields, /plan command with YAML and handler, TUI plan panel rendering with tui-popup, and plan mode input routing (Enter approves, Esc cancels).

Recent closure: replace-production-expect-panics-with-result-propagation (tree.rs: to_snapshot() now returns Result; stream.rs: builder.try_clone() invariant documented; compaction.rs: regex/LazyLock invariant documented; replay.rs: session save gracefully skips tree snapshot on error).

Recent closure: gate-test-support-with-cfg-test (test helper modules now gated `#[cfg(test)]`; helpers duplicated in runie-testing to avoid compile-time dep on tests_support). Recent closure: add-json-file-logging-for-tui (TUI mode now uses JSON file logging with tracing-appender, CLI preserves pretty console output). Recent closure: replace-providerfactory-pinbox-with-async-trait-or-enum (ProviderFactory::validate_key now uses `#[async_trait]` on trait + impls, eliminating `Pin<Box<dyn Future>>` boilerplate). Recent task closures: use-textwrap-for-blockquote-word-wrap (textwrap already used in blockquote rendering), use-atomic-writes-for-config-and-session-files (atomic_write implemented in io/atomic_write.rs), adopt-snapshot-journal-jsonl-pattern (session store already uses JSONL with fs2 advisory locks), generate-tool-dispatch-from-single-registry (BUILTIN_TOOL_NAMES centralized), simplify-permission-policy-chain-to-ruleset-match (PermissionSet ruleset implemented), replace-layered-config-merge-with-figment (Figment already used for config layers). Recent closures: 062-add-tool-result-cache-expiry (tool result cache with TTL expiry in runie-core/tool/cache.rs). Recent closures: await-provider-network-calls-in-ractor-handler (await network calls inline in RactorProviderActor), centralize-provider-http-timeouts-and-retry-constants, centralize-reqwest-client-and-url-normalization, standardize-session-persistence-on-jsonl, derive-event-taxonomies-with-strum-or-proc-macro, unify-bash-execution-into-single-shell-module-with-command-group, unify-form-submit-paths-through-command-registry, unify-sse-parsing-on-openai-frame, replace-emitfn-mutex-with-async-channel, single-config-resolve-default-model, remove-direct-projection-bypasses-in-dispatch-and-domain-ops (use set_git_info/set_cwd_name from domain_ops instead of direct accessor mutation). Recent closure: collapse-durablecoreevent-into-event-enum (Event variants now have transient #[serde(skip)] fields; all test files updated; 2079+ workspace tests pass). Recent closures: update-providerprotocol-doc-to-reflect-wontfix-decision (updated docs to reflect that ProviderProtocol abstraction is kept as documentation/future protocol support), merge-duplicate-tui-widget-replacement-tasks (marked done; input box replacement already complete, form renderer tracked in canonical task), merge-duplicate-session-persistence-tasks (marked done; all referenced tasks were already complete).

Recent closure: add-grok-provider-event-translator-and-headless-replay (added GrokReplayProvider + grok_replay_from_fixtures to runie-testing; 7 headless replay tests pass; Grok SSE parsed via existing replay_sse).
...
Recent sync: Regenerated `tasks/index.json` from task files. Marked 4 remaining Grok Build comparison tasks as `wontfix` (Grok Build unavailable). Recent sync: Regenerated `tasks/index.json` from task files. Total: 198 done, 19 wontfix, 0 partial, 0 blocked, 1 todo (218 total). Recent closure: execute-fifth-pass-ssot-event-protocol-roadmap (fifth-pass meta-task closed; all 18 constituent tasks were done). Recent closure: add-turn-journal-phases-for-crash-recovery (added TurnPhase enum with TurnStarted, ProviderCalled, ToolRequestsRecorded, ResponseDelta, TurnCommitted, TurnAborted phases; TurnPhaseChanged durable event persisted to JSONL; phase tracking in TurnState with reset on turn end; workspace tests pass).
...
Recent closures (spikes): spike-evaluate-async-openai-or-rig-core-for-provider-stack (keep custom stack; adapter complexity ≈ current code), spike-tui-textarea-parity-before-widget-replacement (parity confirmed; all required behaviors covered by tui-textarea 0.7).
...
- **0 `todo` tasks** — see `tasks/index.json` for details.
- **0 `blocked` tasks** — Grok Build comparison tasks marked `wontfix` (Grok Build unavailable); see `tasks/index.json`.
- **0 `partial` tasks** — `make-turnactor-sole-owner-of-turn-state` and `remove-dual-agent-event-application-in-dispatch` marked done (SSOT achieved via idempotency guards); `implement-os-level-bash-sandboxing` is new follow-up for Phase 2 work.
- **439 `done` tasks** — all tasks in `tasks/index.json` are complete.
- **26 `wontfix` tasks** — see `tasks/index.json` for the current list.
