...
- **92 `todo` tasks** — see `tasks/index.json` for details.
- **5 `blocked` tasks** — Grok Build comparison tasks blocked on fixture/recorder/harness readiness; see `tasks/index.json`.
- **2 `partial` tasks** — `live-tui-smoke-test-real-minimax.md` is blocked on a missing `MINIMAX_API_KEY`; `use-color-eyre-and-human-panic-for-cli-errors.md` has a partial implementation.
- **4 `wontfix` tasks** — see `tasks/index.json` for the current list.
