...
UX_HOME="/tmp/runie_ux_home_$$"
UX_SESSIONS="$UX_HOME/sessions"
TMUX="tmux -L runie_ux_$$ -f /dev/null"
...
mkdir -p "$UX_HOME/.runie"
cat > "$UX_HOME/.runie/config.toml" <<'TOML'
provider = "mock"
...
$TMUX new-session -d -s "$SESSION" -x 80 -y 24 "env HOME=$UX_HOME RUNIE_MOCK=1 RUNIE_SESSIONS_DIR=$UX_SESSIONS $BINARY"
sleep 0.5
