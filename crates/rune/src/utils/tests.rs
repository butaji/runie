...
#[cfg(test)]
mod error_tests {
    use crate::{ParseError, RuneError};
    ...
    fn test_rune_error_codegen() {
        let error = RuneError::Codegen("test error".to_string());
    ...
    fn test_rune_result_err() {
        let result: crate::Result<i32> = Err(RuneError::Codegen("test".to_string()));
