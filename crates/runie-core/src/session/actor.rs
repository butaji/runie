...
use crate::session::replay::durable_to_event;
...
impl SessionActor {
    ...
    fn build_meta(&self) -> crate::session::index::SessionMetadata {
        crate::session::index::SessionMetadata {
...
impl SessionActor {
    /// Spawn a session actor that subscribes to the event bus.
    /// Returns an ActorHandle for abort/await.
    pub fn spawn(self, bus: EventBus<Event>) -> ActorHandle {
        let handle = tokio::spawn(self.run_loop(bus));
        ActorHandle {
    ...
    async fn run_loop(mut self, bus: EventBus<Event>) {
...
mod tests {
    ...
    fn spawn_session_actor_test(h: &TestHarness, session_id: &str) -> ActorHandle {
        let actor = SessionActor::new(session_id.into(), "Replay".into(), h.store.clone());
        actor.spawn(h.bus.clone())
    }
    ...
    async fn session_actor_replays_to_uactor() {
        ...
        let handle = spawn_session_actor_test(&h, session_id);
