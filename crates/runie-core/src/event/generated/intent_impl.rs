...
impl Event {
    ...
    pub fn into_intent(self) -> Option<Intent> {
        ...
        match self {
            ...
            Event::RunDeleteCommand { name } => Some(Intent::RunDeleteCommand { name: name.clone() }),
            Event::RunImportCommand { path } => Some(Intent::RunImportCommand { path: path.clone() }),
            Event::RunExportCommand { path } => Some(Intent::RunExportCommand { path: path.clone() }),
            ...
            Event::ExternalEditorDone { content } => Some(Intent::ExternalEditorDone { content: content.clone() }),
            ...
            Event::RenameSession { id, name } => Some(Intent::RenameSession { id: id.clone(), name: name.clone() }),
            ...
            Event::ProvidersSelectModel { provider, model } => Some(Intent::ProvidersSelectModel { provider: provider.clone(), model: model.clone() }),
            Event::ProvidersDisconnect { provider } => Some(Intent::ProvidersDisconnect { provider: provider.clone() }),
            Event::ProvidersAdd => Some(Intent::ProvidersAdd),
            Event::ProvidersEditModels { provider } => Some(Intent::ProvidersEditModels { provider: provider.clone() }),
            ...
            Event::PendingEdit { path, original, proposed } => Some(Intent::PendingEdit { path: path.clone(), original: original.clone(), proposed: proposed.clone() }),
            ...
            Event::ScopedModelToggle { provider, name } => Some(Intent::ScopedModelToggle { provider: provider.clone(), name: name.clone() }),
            Event::ScopedModelEnableAll => Some(Intent::ScopedModelEnableAll),
            Event::ScopedModelDisableAll => Some(Intent::ScopedModelDisableAll),
            Event::ScopedModelToggleProvider { provider } => Some(Intent::ScopedModelToggleProvider { provider: provider.clone() }),
            ...
            Event::SettingsSwitchCategory { ref category } => Some(Intent::SettingsSwitchCategory { category: *category }),
            ...
            Event::PermissionResponse { ref request_id, ref action } => Some(Intent::PermissionResponse { request_id: request_id.clone(), action: *action }),
