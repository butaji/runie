...
mod tests {
    ...
    fn move_cursor_up_first_line_flashes() {
        let mut state = AppState::default();
        state.input.input = "line1\nline2".to_string();
        state.input.cursor_pos = 2; // On first line

        state.move_cursor_up();
        assert_eq!(state.input.input_flash, 3); // Flash indicator
    }
    ...
    fn move_cursor_up_navigates() {
        let mut state = AppState::default();
        state.input.input = "line1\nline2".to_string();
        state.input.cursor_pos = 8; // On second line (column 2)

        state.move_cursor_up();
        assert_eq!(state.input.cursor_pos, 2); // First line, same column
    }
    ...
    fn move_cursor_down_last_line_flashes() {
        let mut state = AppState::default();
        state.input.input = "line1\nline2".to_string();
        state.input.cursor_pos = 8; // On last line
    ...
    fn move_cursor_down_navigates() {
        let mut state = AppState::default();
        state.input.input = "line1\nline2\nline3".to_string();
        state.input.cursor_pos = 2; // On first line (column 2)

        state.move_cursor_down();
        assert_eq!(state.input.cursor_pos, 8); // Second line, same column

        state.move_cursor_down();
        assert_eq!(state.input.cursor_pos, 14); // Third line, same column
    }
    ...
    fn cursor_preserves_column_when_moving() {
        ...
        state.input.input = "ab\nabcdef".to_string();
        state.input.cursor_pos = 6; // On second line, column 3 ('d')

        state.move_cursor_up();
        // First line has 2 chars, column 3 clamps to 2
        assert_eq!(state.input.cursor_pos, 2); // End of first line
    ...
    fn column_clamped_to_line_length() {
        let mut state = AppState::default();
        state.input.input = "ab\nabcdef".to_string();
        state.input.cursor_pos = 10; // On second line, column 6
