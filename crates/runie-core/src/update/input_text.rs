//! Text editing methods and grapheme-aware helpers for the input field.

use super::input_text_support as support;
...
impl AppState {
    ...
    fn remove_grapheme_before_cursor(&mut self) {
        self.push_undo();
        let new_pos = support::prev_grapheme_boundary(&self.input.input, self.input.cursor_pos);
    ...
    pub(crate) fn delete_word(&mut self) {
        ...
        self.push_undo();
        let start = support::find_word_boundary_left(&self.input.input, self.input.cursor_pos);
    ...
    pub(crate) fn kill_char(&mut self) {
        if self.input.cursor_pos < self.input.input.len() {
            self.push_undo();
            let end = support::next_grapheme_boundary(&self.input.input, self.input.cursor_pos);
    ...
    fn take_submit_content(&mut self) -> Option<String> {
        ...
        self.input.redo_stack.clear();
        if support::is_quit_command(&content) {
    ...
    fn apply_command_result(&mut self, result: crate::commands::CommandResult) {
        ...
        match result {
            ...
            crate::commands::CommandResult::OpenDialog(d) => match d {
                DialogType::CommandPalette => super::dialog::open_command_palette(self),
                DialogType::ModelSelector => super::dialog::open_model_selector(self),
                DialogType::Settings => super::dialog::open_settings_dialog(self),
                DialogType::ScopedModels => super::dialog::open_scoped_models_dialog(self),
            },
    ...
    fn run_bash_command(&mut self, command: &str) {
        let result = super::bash::execute_bash(command);
