    ...
    match event {
        ...
        Event::HistoryPrev | Event::PaletteUp => {
            let new_sel = if selected == 0 {
                state.scoped_models.len().saturating_sub(1)
            } else {
                selected - 1
            };
        ...
        Event::HistoryNext | Event::PaletteDown => {
            let new_sel = if state.scoped_models.is_empty() {
                0
            } else {
                (selected + 1) % state.scoped_models.len()
            };
            state.open_dialog = Some(DialogState::ScopedModels { selected: new_sel });
            state.mark_dirty();
        }
        Event::Submit | Event::PaletteSelect => {
            if let Some(model) = state.scoped_models.get(selected) {
...
pub fn toggle_scoped_model(state: &mut AppState, name: &str) {
    if let Some(idx) = state.scoped_models.iter().position(|m| m.name == name) {
        state.scoped_models[idx].enabled = !state.scoped_models[idx].enabled;
        state.mark_dirty();
    }
}

pub fn enable_all(state: &mut AppState) {
    for m in &mut state.scoped_models {
        m.enabled = true;
    }
    state.mark_dirty();
}
