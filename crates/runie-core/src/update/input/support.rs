...
pub fn is_quit_command(content: &str) -> bool {
    content.eq_ignore_ascii_case("quit") || content.eq_ignore_ascii_case("exit") || content.eq_ignore_ascii_case(":q")
}
...
pub fn at_suggestion_hints() -> Vec<String> {
    vec!["Tab cycle".to_owned(), "Enter insert".to_owned(), "Esc close".to_owned()]
}

pub fn input_active_hints() -> Vec<String> {
    vec!["Enter send".to_owned(), "Alt+Enter follow-up".to_owned(), "Esc clear".to_owned()]
}
...
pub fn modal_hints() -> Vec<String> {
    vec!["↑/↓ select".to_owned(), "Enter confirm".to_owned(), "Esc close".to_owned()]
}
