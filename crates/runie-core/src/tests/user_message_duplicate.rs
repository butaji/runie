...
fn test_single_user_message_in_feed() {
    ...
    println!("Session messages: {:?}", state.session().messages.iter().map(|m| (m.role.clone(), m.content())).collect::<Vec<_>>());
    println!("User messages in session: {}", user_messages.len());
    println!("Feed elements: {:?}", feed.elements.iter().map(|e| format!("{:?}", e)).collect::<Vec<_>>());
...
fn test_full_submit_flow() {
    ...
    println!("After submit - user messages in session: {}", user_messages.len());
    ...
    println!("\nFeed elements: {:?}", feed.elements.iter().map(|e| format!("{:?}", e)).collect::<Vec<_>>());
    println!("UserMessage elements in feed: {}", user_elements.len());
...
fn test_apply_user_message_twice_should_not_duplicate() {
    ...
    println!("After applying same message twice - user messages: {}", user_messages.len());
    for (i, msg) in state.session().messages.iter().enumerate() {
