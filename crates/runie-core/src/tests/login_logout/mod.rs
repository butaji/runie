...
pub(crate) fn default_models_for_provider(provider: &str) -> Vec<String> {
    crate::provider::find_provider(provider)
        .map(|p| p.models.iter().map(|m| m.name.to_string()).collect())
        .unwrap_or_default()
}

pub(crate) fn validate_provider(state: &mut crate::model::AppState, provider: &str, key: &str) {
    let models = default_models_for_provider(provider);
    fetch_models_for(state, provider, key, &models);
}

pub(crate) fn add_minimax_provider(state: &mut crate::model::AppState) {
