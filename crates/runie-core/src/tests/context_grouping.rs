...
fn single_context_tool_is_not_grouped() {
    ...
    state.session.messages.push(tool_message("list_dir", "a.txt", 1.0));
    state.messages_changed();

    let elems = LazyCache::rebuild(&state);
    assert!(
        elems.iter().any(|e| matches!(e, Element::ToolDone { name, .. } if name == "list_dir")),
        "single context tool should remain a ToolDone element"
    );
    assert!(
        !elems.iter().any(|e| matches!(e, Element::ContextGroup { .. })),
...
fn multiple_context_tools_are_grouped() {
    let mut state = AppState::default();
    state.session.messages.push(tool_message("list_dir", "a.txt", 1.0));
    state.session.messages.push(tool_message("read_file", "content", 2.0));
    state.session.messages.push(tool_message("grep", "match", 3.0));
    state.messages_changed();
    ...
    let groups: Vec<_> = elems
        ...
        .collect();
    assert_eq!(groups, vec![3], "three consecutive context tools should form one group");
    assert!(
...
fn action_tool_breaks_context_group() {
    let mut state = AppState::default();
    state.session.messages.push(tool_message("list_dir", "a.txt", 1.0));
    state.session.messages.push(tool_message("execute_command", "out", 2.0));
    state.session.messages.push(tool_message("read_file", "content", 3.0));
    state.messages_changed();
    ...
    assert!(
        elems.iter().any(|e| matches!(e, Element::ToolDone { name, .. } if name == "execute_command")),
        "action tool should remain a standalone ToolDone"
    );
    assert!(
        elems.iter().any(|e| matches!(e, Element::ToolDone { name, .. } if name == "read_file")),
...
fn assistant_parts_render_into_elements() {
    ...
    let elems = LazyCache::rebuild(&state);
    assert!(elems.iter().any(|e| matches!(e, Element::AgentMessage { content, .. } if content == "Let me search.")));
