...
fn agent_response_updated_after_tool_stays_after_tool() {
    ...
    assert!(
        ...
        "Agent should appear after Tool when response continues after tool: got {:?}",
        kinds
    );
}
...
fn thought_appears_before_agent_even_when_agent_updated_later() {
    ...
    assert!(
        ...
        "Thought should appear before Agent even when Agent timestamp is bumped later: got {:?}",
        kinds
    );
}
...
fn previous_turn_complete_before_next_turn_user() {
    ...
    let turn_pos = kinds
        ...
        .position(|k| k == "Turn")
        .expect("TurnComplete");
    let _user2_pos = kinds
        .iter()
        .position(|k| k == "User" && *k != "User")
        .unwrap_or(0);
    ...
    assert!(
        ...
        "TurnComplete of turn 1 should be before user message of turn 2: got {:?}",
        kinds
    );
}
