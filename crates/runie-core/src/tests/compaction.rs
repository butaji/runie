...
fn session_tokens_from_messages() {
    ...
    state.session.messages.push(ChatMessage { role: Role::User, content: "Hello world test".to_string(), timestamp: 0.0, id: "u1".into(), ..Default::default()});
    state.session.messages.push(ChatMessage { role: Role::Assistant, content: "Response here".to_string(), timestamp: 0.0, id: "a1".into(), ..Default::default()});
...
fn compaction_does_not_cut_mid_turn() {
    let mut state = AppState::default();
    state.session.messages.push(ChatMessage { role: Role::User, content: "Start".to_string(), timestamp: 0.0, id: "u1".into(), ..Default::default()});
    state.session.messages.push(ChatMessage { role: Role::Assistant, content: "A".to_string(), timestamp: 1.0, id: "a1".into(), ..Default::default()});
    state.session.messages.push(ChatMessage { role: Role::Tool, content: "tool result".to_string(), timestamp: 2.0, id: "t1".into(), ..Default::default()});
    state.session.messages.push(ChatMessage { role: Role::User, content: "Recent".to_string(), timestamp: 3.0, id: "u2".into(), ..Default::default()});
    state.compact(10);
