...
fn transient_success_has_expiry() {
    ...
    assert!(state.transient_until.is_some(), "Success message should have expiry");
}
...
fn transient_error_has_no_expiry() {
    ...
    assert!(state.transient_until.is_none(), "Error message should NOT have expiry");
}

#[test]
fn transient_system_message_has_expiry() {
    let mut state = fresh_state();
    state.update(Event::System(SystemEvent::SystemMessage {
        content: "info".to_string(),
    }));
    assert!(state.transient_until.is_some(), "System message should have expiry");
}
...
fn transient_message_with_different_levels() {
    ...
    assert_eq!(state.transient_level, Some(crate::event::TransientLevel::Info));
...
fn transient_expiry_time_is_reasonable() {
    ...
    if let Some(expiry) = state.transient_until {
        ...
        assert!(expiry.duration_since(now).as_secs() <= 10, "Expiry should be reasonable");
    }
}
