use tidy_agent::{Agent, CodingAgent, AgentConfig, AgentLoop, AgentState};
use tidy_ai::providers::OpenAiProvider;
use tidy_tools::{Workspace, create_default_toolkit};
use tidy_router::Router;
use tidy_orchestrator::Orchestrator;
use tidy_core::Session;
use std::sync::Arc;
    ...
    pub async fn run_interactive(&mut self) -> Result<(), AppError> {
        ...
        loop {
            ...
            if let Some(agent) = &mut self.agent {
                ...
                for event in events {
                    match event {
                        tidy_core::Event::MessageDelta { content } => print!("{}", content),
                        tidy_core::Event::ToolExecutionStart { tool_name, .. } => {
                            println!("\n[Executing: {}]", tool_name);
                        }
                        tidy_core::Event::Error { message } => {
                            eprintln!("\n[Error: {}]", message);
                        }
                        _ => {}
                    }
