...
// ─── Message feed timestamps and user bg ────────────────────────────────
...
fn user_message_has_border_bg_color() {
    ...
    terminal.draw(|f| view(f, &mut state)).unwrap();
    let buf = terminal.backend().buffer();
    let border_color = crate::theme::color_border();

    let mut found_user_bg = false;
    for y in 0..buf.area().height {
        for x in 0..buf.area().width {
            if buf[(x, y)].symbol() == "h" && buf[(x, y)].style().bg == Some(border_color) {
                found_user_bg = true;
            }
        }
    }
    assert!(found_user_bg, "User message should have border color as background");
}
...
fn timestamp_shown_once_per_message_element() {
    ...
    assert_eq!(timestamp_count, 1, "Timestamp should appear exactly once per message element, got {}", timestamp_count);
}
...
fn timestamp_right_aligned_on_first_line() {
    ...
    assert!(found_right, "Should find timestamp in buffer");
}
...
fn agent_message_timestamp_appears_once() {
    ...
    assert_eq!(timestamp_count, 1, "Agent message timestamp should appear once, got {}", timestamp_count);
}
