...
#[test]
fn user_message_card_line_count() {
    let msg = ChatMessage {
        role: Role::User,
    ...
    assert_eq!(
        ...
        "UserMessage card (bg-padding + content + bg-padding = 3) + Spacer (1) = 4 lines"
    );
}
...
fn scroll_up_shows_older_content() {
    ...
    // 5 messages = 20 lines total (5*3 card + 5 spacers). Viewport of 3 lines.
    // scroll=8: viewport [8, 11) — msg2 card visible, msg4 hidden
    state.view.scroll = 8;
