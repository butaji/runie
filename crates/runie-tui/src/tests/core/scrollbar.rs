...
fn visible_uses_scroll_offset() {
    ...
    assert!(
        visible_bottom.elements.iter().any(
            |e| matches!(e, runie_core::view::elements::Element::UserMessage { content, .. } if content == "msg9")
        ),
        "Bottom should show latest"
    );
    ...
    assert!(
        visible_top.elements.iter().any(
            |e| matches!(e, runie_core::view::elements::Element::UserMessage { content, .. } if content == "msg0")
        ),
        "Top should show oldest"
    );
}
