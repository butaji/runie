...
use runie_core::{AppState, dialog::{Panel, PanelStack, ItemAction}, commands::DialogState};
use crate::ui::view;
...
fn form_has_descriptive_header() {
    ...
    let has_empty_filter = lines.iter().any(|l| l.trim() == "❯" || (l.trim_start().starts_with('❯') && l.trim().len() <= 2));
    assert!(!has_empty_filter, "Form should not show empty filter line, got: {:?}", lines);

    // Should have some form-related header
    let has_form_header = lines.iter().any(|l|
        l.contains("form") || l.contains("Field") || l.contains("fill"));
    assert!(has_form_header, "Form should have a descriptive header, got: {:?}", lines);
}
...
fn form_field_label_above_input() {
    ...
    let name_idx = lines.iter().position(|l| l.contains("Name"))
        .expect("Form should have a 'Name' label");

    // The line(s) AFTER the label should contain the input value/placeholder
    let value_line = lines.iter().skip(name_idx + 1).find(|l| l.contains("session-name") || l.contains('│'));
...
fn form_shows_progress_indicator() {
    ...
    assert!(has_progress,
        "Form should show a progress indicator (e.g. ① ②, 1/2, 'Field 1 of 2'), got: {:?}", lines);
}
...
fn form_has_prominent_submit_button() {
    ...
    let submit_line = lines.iter().find(|l| l.contains("Submit"))
        .expect("Form should have a Submit button");

    // The Submit line should have either:
    // - Box-drawing markers (rounded button look)
    // - Or have an arrow ▶
    let has_prominence = submit_line.contains('▶') ||
                         submit_line.contains('╭') || submit_line.contains('╰') ||
                         submit_line.contains('┌') || submit_line.contains('└') ||
                         submit_line.contains('│');
    assert!(has_prominence,
        "Submit button should be visually prominent (box/arrow markers), got: '{}'", submit_line);
}
...
fn form_active_field_visually_distinct() {
    ...
    assert!(has_cursor,
        "Active field should have a visual cursor indicator, got: {:?}", lines);
}
...
fn form_completed_fields_show_checkmark() {
    ...
    let mut state = AppState::default();
    let panel = Panel::new("save", "Save Session")
        .form_field_value("Name", "session-name", "name", "my-session");
    open_form_dialog(&mut state, panel);
    ...
    let has_filled = lines.iter().any(|l|
        l.contains('✓') || l.contains('✔') || l.contains("my-session")
    );
    assert!(has_filled,
        "Filled field should show value 'my-session' or checkmark, got: {:?}", lines);
}
...
fn form_hotkeys_pinned_to_bottom() {
    ...
    let buf = render(&mut state);
    let inner = popup_inner_rect();

    // Check the last 2 lines of the inner area for the hotkey hint
    let last_y = inner.y + inner.height - 1;
    let last_line: String = (inner.x..inner.x + inner.width)
        .map(|x| buf[(x, last_y)].symbol())
        .collect();

    assert!(
        last_line.contains("navigate") || last_line.contains("edit") || last_line.contains("esc"),
        "Hotkey hint should be on the LAST line of the popup, got: '{}'", last_line
    );
}
