use runie_core::{AppState, ChatMessage, Role, Event};
use crate::ui::view;
...
fn scrollbar_renders_in_rightmost_column() {
    ...
    for y in 0..buf.area().height {
        let rightmost = buf.get(39, y).symbol();
        if rightmost == thumb {
            found_at_right_edge = true;
        }
    }
    assert!(found_at_right_edge, "Scrollbar thumb '{}' should be in rightmost column (x=39)", thumb);
}
...
fn content_uses_full_width_when_scrollbar_present() {
    ...
    for y in 0..buf.area().height {
        let sym = buf.get(18, y).symbol();
        if sym != " " && sym != crate::theme::SCROLLBAR_THUMB {
            found_content_near_edge = true;
        }
    }
    assert!(found_content_near_edge, "Content should reach near right edge (col 18) — scrollbar is in margin, not content area");
}
...
fn no_scrollbar_when_content_fits() {
    ...
    let thumb = crate::theme::SCROLLBAR_THUMB;
    for y in 0..buf.area().height {
        let sym = buf.get(39, y).symbol();
        assert_ne!(sym, thumb,
            "No scrollbar thumb at y={} when content fits", y);
    }
}
...
fn scrollbar_thumb_uses_dimmed_style() {
    ...
    for y in 0..buf.area().height {
        let cell = buf.get(39, y);
        if cell.symbol() == thumb {
            assert_eq!(cell.style().fg, Some(dim_fg),
...
fn scrollbar_track_is_invisible() {
    ...
    assert!(found_track, "Should find at least one track ('{}') cell in scrollbar column", track);
}
