...
use runie_core::{AppState, Event, model::ScopedModel};
use crate::ui::view;
...
fn hotkeys_visible_when_list_overflows() {
    ...
    assert!(rect_contains_text(&buf, popup_rect, "navigate"),
        "Hotkey hint should be visible inside the dialog popup area");
    assert!(rect_contains_text(&buf, popup_rect, "toggle"),
        "Hotkey hint should mention 'toggle' keybinding");
    assert!(rect_contains_text(&buf, popup_rect, "close"),
...
fn hotkeys_pinned_to_bottom_of_popup() {
    ...
    assert!(rect_contains_text(&buf, bottom_rect, "navigate"),
        "Hotkey hint should be in the bottom 2 lines of the popup");
}

/// Hotkeys should be styled (not raw text) — using the shared hint parser.
#[test]
fn hotkeys_use_styled_key_indicator() {
    ...
    let buf = render_dialog(&mut state);
    let last = popup_inner_rect().y + popup_inner_rect().height - 1;
    let key_fg = crate::theme::style_hint_key().fg;
    let mut found_styled = false;
    for x in popup_inner_rect().x..popup_inner_rect().x + popup_inner_rect().width {
        let cell = &buf[(x, last)];
        if cell.symbol() != " " && cell.style().fg == key_fg {
            found_styled = true;
            break;
        }
    }
    assert!(found_styled, "Hotkey keys should be styled with style_hint_key()");
}
