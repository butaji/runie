...
use runie_agent::{PermissionDecision, AgentMessage, ContentPart};
...
pub fn handle_permission(state: &mut AppState, decision: PermissionDecision) -> Vec<crate::tui::state::Cmd> {
    ...
    if should_rollback {
        if let Some(id) = tool_call_id {
            cmds.push(Cmd::Rollback { tool_call_id: id });
        }
    }
...
pub fn handle_permission_msg(state: &mut AppState, msg: Msg) -> Vec<crate::tui::state::Cmd> {
...
pub fn handle_permission_timeout(state: &mut AppState) -> Vec<crate::tui::state::Cmd> {
    use crate::tui::state::Cmd;
