...
pub(crate) fn apply_trust_on_startup(state: &mut AppState) {
...
pub(crate) fn init_skills(state: &mut AppState) {
    state.skills = runie_core::skills::load_all();
}

pub(crate) fn init_prompts(state: &mut AppState) {
    let config = config_reload::Config::load_from(&config_reload::config_path());
...
pub(crate) fn init_truncation(state: &mut AppState) {
    let config = config_reload::Config::load_from(&config_reload::config_path());
