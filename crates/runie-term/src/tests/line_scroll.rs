...
fn large_thought_clipped_from_top_not_bottom() {
    let mut state = AppState::default();
    state.update(Event::AgentResponse { id: "req.0".into(), content: "before".into() });
    state.update(Event::AgentThinking { id: "req.0".into() });
    let mut thought = "◆ Thought 1.0s\n".to_string();
    for i in 1..=15 {
        thought.push_str(&format!("line{}\n", i));
    }
    state.update(Event::AgentResponse { id: "req.0".into(), content: thought });
    ...
    assert!(out.contains("line15"), "Bottom lines of thought should be visible");
}
...
fn scroll_up_shows_older_content() {
    ...
    assert!(!out.contains("msg19"), "Latest message should be hidden after scrolling up");
}
...
fn new_message_pushes_old_upward() {
    ...
    assert!(!after.contains("msg0"), "msg0 should be pushed off-screen by newer messages");
...
fn partial_element_at_top_when_overflow() {
    ...
    state.update(Event::AgentResponse { id: "req.1".into(), content: thought });
...
fn at_bottom_auto_scrolls_to_show_new() {
    ...
    let out = render_content(&mut state);
    assert!(out.contains("hello"), "Newly submitted message must be visible");
}
