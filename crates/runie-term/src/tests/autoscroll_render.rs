...
fn latest_user_message_visible_after_submit() {
    ...
    assert!(out.contains("list files"), "Submitted message must be visible");
}
...
fn scroll_up_shows_older_content() {
    ...
    assert!(!out.contains("msg14"), "Latest message hidden after scroll up");
}
...
fn new_content_auto_shows_when_at_bottom() {
    ...
    let out = render_chat(&mut state, 40, 15);
    assert!(out.contains("NEWEST"), "New content must be visible when at bottom");
}
