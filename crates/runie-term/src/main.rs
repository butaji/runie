...
async fn simple_flow(cmd: &AgentCommand, agent_tx: &mpsc::Sender<CoreEvent>) {
    ...
    for chunk in provider.generate(msgs) {
        let _ = agent_tx.send(CoreEvent::AgentResponse {
            id: cmd.id.clone(), content: chunk.content,
        }).await;
        sleep(50).await;
    }
}
...
async fn tool_flow(cmd: &AgentCommand, agent_tx: &mpsc::Sender<CoreEvent>) {
    ...
    let _ = agent_tx.send(CoreEvent::AgentResponse {
        id: cmd.id.clone(), content: "Files:\n".to_string(),
    }).await;
    sleep(50).await;

    let _ = agent_tx.send(CoreEvent::AgentTurnComplete {
        id: cmd.id.clone(), duration_secs: start.elapsed().as_secs_f64(),
    }).await;
}
...
async fn tool_exec(id: &str, agent_tx: &mpsc::Sender<CoreEvent>) {
    let _ = agent_tx.send(CoreEvent::AgentToolStart {
        id: id.to_string(), name: "list_files".to_string(),
    }).await;
    sleep(1000).await;
