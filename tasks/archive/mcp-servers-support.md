...
- [x] `McpServerConfig` type: `name`, `command`, `args`, `env`
- [x] Load MCP servers from `~/.runie/mcp.toml` (or `config.mcp` section)
- [x] `McpStatus` enum: `Connected`, `Disconnected`, `Unavailable`
- [ ] `/mcps` slash command opens the MCP servers panel
- [ ] Each server shows: name, status badge, command preview
- [x] Auto-injected managed MCP credentials (`RUNIE_MCP_<NAME>_TOKEN` env vars)
- [x] Status indicator `⛔ N MCP servers unavailable` shown when servers are down
...
### Layer 1 — State/Logic
- [x] `parse_mcp_config_toml` — parses `[[mcp.servers]]` array
- [x] `mcp_server_status_from_connection` — connection state → status enum
- [x] `inject_mcp_env_vars` — adds `RUNIE_MCP_<NAME>_TOKEN` for each server
- [x] `filter_available_servers` — returns only connected servers
- [x] `count_unavailable_servers` — returns count of disconnected/unavailable
...
- [x] `unavailable_badge_renders` — `⛔ N MCP servers unavailable` shows when count > 0
