...
Six modules in `crates/runie-core/src/` use the dual-path layout (`foo.rs` declaring `mod foo/` submodules) instead of the conventional `foo/mod.rs` style. The dual-path form is non-idiomatic and forces `rg`/glob/grep users to remember two locations per module. Pick one style workspace-wide and migrate.
...
Rust 2018+ supports both layouts; this is purely a consistency change. After migration, every module is a single `foo/mod.rs` + siblings.

## Acceptance Criteria

- [ ] For each of the 6 pairs, `foo.rs` becomes `foo/mod.rs` (its content is unchanged except for relative path fixes if any).
...
### Layer 4 — Smoke / Crash
- [ ] `smoke_module_paths_resolve_after_consolidation` — `cargo check --workspace` green; `rg "crate::tool_markers|crate::tool_parser|crate::provider_registry|crate::model_catalog|crate::login_config|crate::message" crates/` returns the same set of callers as before.
