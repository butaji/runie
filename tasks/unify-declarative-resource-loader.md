...
- `skills/load.rs:9–182` scans a directory for `SKILL.md` subdirectories and flat `.md` files, extracts YAML frontmatter, resolves names, and builds `Skill` records.
- `declarative/loader.rs:67–259` repeats the same pattern for `SkillDef` (and also loads YAML commands separately).
- Both use their own `extract_frontmatter` helpers and name-derivation rules.
...
- [ ] A shared loader module exists (e.g., `crates/runie-core/src/resource_loader.rs` or under `declarative/`) that handles directory scanning, `SKILL.md` subdirectory precedence, flat `.md` files, YAML frontmatter extraction, and name resolution.
...
### Layer 4 — Smoke / Crash
- [ ] `load_all_skills_no_panic` — loading the bundled `resources/agents/` directory (or a temporary equivalent) does not panic after the refactor.
...
- Out of scope: unifying the command YAML loader or changing the resource file format.
