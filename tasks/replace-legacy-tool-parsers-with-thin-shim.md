...
- [ ] Replace MiniMax XML parsing with `quick-xml` inside the new shim.
- [ ] Centralize JSON object detection into a single helper used by the shim.
...
- [ ] `text_provider_tool_call_shim` — replays a captured provider SSE fixture through the new shim and confirms tool calls, arguments, and markers are stripped identically to the legacy path.
