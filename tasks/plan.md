...
5. MCP lifecycle ownership — `harness-10`; unified MCP status rows now own their stable terminal projection, so `/mcps` does not rebuild transport/index/status formatting in the TUI.
...
8. Diagnostics and media completeness — `harness-16` and `harness-19`; diagnostic report summaries now carry typed inspect/fix action data alongside their stable rows, and Anthropic image URLs now use the provider’s remote URL source shape.
