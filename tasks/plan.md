...
8. Diagnostics and media completeness — `harness-16` and `harness-19`; diagnostic report summaries now carry typed inspect/fix action data alongside their stable rows, Anthropic image URLs now use the provider’s remote URL source shape, and user-question dialogs now project optional header/body metadata as data.
