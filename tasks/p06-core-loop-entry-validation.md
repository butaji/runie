...
- `Agent.continue()` — `agent.ts:350`: throws if busy; if last message is assistant, drains steering then follow-up (runs those), else `runContinuation()`.
...
2. Add validation to `run_loop_continue`:
   ...
   - last message is `Assistant` → error `"Cannot continue from message role: assistant"`.
