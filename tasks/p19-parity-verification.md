...
the ordered lifecycle subsequence without sleeps. Its `pi_events` assertion
now also checks the closed Pi wire projection, not only the compatibility
event names. This closes the previously Rust-only YAML gap for the Pi abort
contract; scheduler-driven mid-stream abort timing remains covered by the
dedicated async core tests.
...
the synchronous compatibility renderer state mirror after all replay callers
are actor-backed (the live app's tool and assistant projections are now
actor-owned), and (2) obtaining a clean Grok full-color capture for strict
