...
- [ ] **Actor/SSOT:** `PermissionActor` owns permission registry; tool annotations are metadata.
- [ ] **Trigger events:** `PermissionRequest` triggers permission check.
- [ ] **Observer events:** `PermissionResponse` notifies observers of permission decision.
- [ ] **No direct mutations:** Permission decisions must go through `PermissionActor`.
- [ ] **No new mirrors:** Tool metadata is authoritative in tool registry; no duplicates.
- [ ] **Async work observed:** N/A (synchronous permission checks).
