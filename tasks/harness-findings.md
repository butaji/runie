...
15. `harness-15-user-questions` — **partial** — structured validation, owned broker, live selector, multi-select answers, YAML replay, explicit broker cancellation, broker-side option validation, bounded resolution traces, and bounded JSONL trace export/restore for answered/cancelled/rejected requests are implemented; integration into the broader session journal remains.
...
18. `harness-18-noninteractive` — **partial** — typed JSONL event encoding/decoding, deterministic completed/aborted/failed exit codes, pure `--jsonl`/explicit approval argument parsing, and the `runie --jsonl --prompt` loop boundary now exist; stdin streaming and richer provider event forwarding remain.
