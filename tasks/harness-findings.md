...
19. `harness-19-diagnostics` — **partial** — actor-owned `/doctor` now projects a serializable report with explicit checks and fix intent, and telemetry exposes a serializable usage summary over ended provider spans; tracing export bundles and visualization remain.
