...
2. `harness-02-permissions` — **partial** — pure Ask/Auto/YOLO policy now routes live mutating tools through the existing owned question dialog, and approval commands project into the live mode store; deny mode and replay fixtures remain.
3. `harness-03-shell-lifecycle` — **partial** — Bash now streams stdout/stderr chunks and returns separate stdout/stderr/exit metadata; owned background process actors now track start/completion/failure/cancellation, while TUI controls remain.
