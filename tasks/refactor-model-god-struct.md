...
- [x] `InputState` struct extracted with its methods from `update/input.rs`
- [x] `AgentState` struct extracted with agent-related fields
- [x] `ViewState` struct extracted with cache/render-related fields
- [x] `AppState` becomes a container struct holding sub-structs by value
- [x] `Snapshot::new_from_state()` updated to copy from sub-structs
- [x] All field accesses via `app.input.input.clone()` etc. or provide accessor methods
- [x] All existing tests pass

## Tests

### Layer 1 — State/Logic
- [x] `test_input_state_undo_redo` — verify undo/redo still works
- [x] `test_agent_state_queue_operations` — verify queue operations
- [x] `test_view_state_cache_invalidation` — verify dirty tracking

### Layer 2 — Event Handling
- [x] All existing event tests pass

### Layer 3 — Rendering
- [x] `test_snapshot_captures_sub_state` — verify snapshot contains all fields

### Layer 4 — Smoke
- [x] `smoke_session_lifecycle.sh` — create, interact, save session
