...
- **Already migrated and wired to production:** `InputActor`, `RactorPermissionActor`, and `RactorConfigActor`.
...
2. **Migrate `IoActor` and `SessionActor`** in either order.
3. **Migrate `FffIndexerActor`**; keep a small wrapper if the static `FffSearchState` service-locator remains.
4. **Migrate `AgentActor`** in `runie-agent`; introduce a factory trait in `runie-core` to avoid a crate-dependency cycle.
...
- [ ] All production spawn sites (`runie-tui/src/main.rs`, `runie-cli/src/acp.rs`, `Leader::start`, `HeadlessRuntime`) use the ractor-based versions for the actors listed above.
...
- [ ] `ractor_io_actor_writes_file` — `IoActor` handles a write request through ractor.
- [ ] `ractor_session_actor_stores_turn` — `SessionActor` stores and retrieves a turn through ractor.
...
- `crates/runie-core/src/actors/session/ractor_session.rs` (new)
...
- `crates/runie-core/src/testing/actor_harness.rs` currently depends on the custom `Actor` trait; update its counter-actor tests to use `ractor` so that `trait.rs` can actually be deleted.
