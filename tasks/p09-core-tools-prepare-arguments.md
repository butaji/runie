...
- `AgentTool` trait (types.rs:337) has `validate_arguments(&self, &Value) -> Result<(), String>` (trait default Ok), but **no** `prepare_arguments`.
- `tools/executor.rs` `validate_args` runs preflight but does not implement the JSON-schema coercion/error format.
